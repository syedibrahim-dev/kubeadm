...
# Run as ubuntu user
if su - ubuntu -c "cd /home/ubuntu/kubeadm-infra && terraform init && terraform apply -var-file=config/terraform.tfvars -var='deploy_argocd=true' -target='module.argocd[0]' -auto-approve" >> /var/log/argocd-deploy.log 2>&1; then
