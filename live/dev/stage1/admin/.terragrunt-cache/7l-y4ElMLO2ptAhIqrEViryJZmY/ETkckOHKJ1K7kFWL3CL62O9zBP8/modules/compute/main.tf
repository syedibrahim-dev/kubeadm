...
resource "aws_instance" "control_plane" {
  ...
  user_data = var.enable_auto_setup ? templatefile("${path.module}/../../scripts/control-plane-setup.sh", {
...
resource "aws_instance" "worker" {
  ...
  # Automatic Kubernetes setup
  user_data = var.enable_auto_setup ? templatefile("${path.module}/../../scripts/worker-setup.sh", {
    control_plane_ip   = var.control_plane_private_ip
    control_plane_name = var.control_plane_name
    aws_region         = var.aws_region
    k8s_version        = var.k8s_version
